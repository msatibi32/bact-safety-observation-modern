...
export default function BrandHeader({ title, titleEn, subtitle, subtitleEn, size = 'md', className = '' }) {
  return (
    <header className={`flex flex-col items-center text-center ${className}`}>
      ...
      {titleEn && <p className="mt-0.5 text-sm font-medium text-slate-400">{titleEn}</p>}
      {subtitle && <p className="mt-1 max-w-sm text-sm text-slate-500">{subtitle}</p>}
      {subtitleEn && <p className="mt-0.5 max-w-sm text-xs text-slate-400">{subtitleEn}</p>}
    </header>
  )
}
