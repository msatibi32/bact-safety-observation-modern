    ...
    <div className="space-y-4">
      ...
      <ul className="space-y-3">
        {items.map((capa) => {
          ...
          return (
            <li key={capa.id} className={`rounded-xl border p-3 ${overdue ? 'border-red-500/30 bg-red-500/10' : 'border-slate-700 bg-slate-800/50'}`}>
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  {capa.description && <p className="mt-1 text-sm text-slate-400">{capa.description}</p>}
                </div>
      ...
      <form onSubmit={handleAdd} className="space-y-2 border-t border-slate-800 pt-4">
